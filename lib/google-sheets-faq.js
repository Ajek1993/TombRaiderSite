  ...
  try {
    ...
    return rows.map((row, index) => {
      return {
        ...
        visible: row[5] === 'true' || row[5] === true || row[5] === '1',
...
async function addFAQ(faq) {
  try {
    ...
    const row = [
      ...
      faq.visible !== undefined ? faq.visible : true,
...
async function updateFAQ(id, updates) {
  try {
    ...
    const updatedRow = [
      ...
      updates.visible !== undefined ? updates.visible : (current[5] === 'true' || current[5] === true || current[5] === '1'), // column F
